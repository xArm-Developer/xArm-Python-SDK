...
def connect_report_realt(ip):
    ...
    while (1):
        ...
        if rx_data != -1 and len(rx_data) > 5:
            rxcnt += 1
            print("【real   report】: len = %d, rxcnt = %d" % (len(rx_data), rxcnt), time.time())
            print_report_norm(rx_data)
            print(" ")
        time.sleep(0.001)
...
def main():
    ...
    ret = arm_cmd.motion_en(8, 1)
    print('motion_en, ret = %d' % (ret[0]))

    ret = arm_cmd.set_state(0)
    print('set_state, ret = %d' % (ret[0]))

    ret = arm_cmd.move_gohome(10/57, 100, 0)
    print('move_gohome, ret = %d' % (ret[0]))

    ret = arm_cmd.sleep_instruction(0.1)
    print('sleep_instruction, ret = %d' % (ret[0]))

    pose = [[300, 0,    100, -3.14, 0, 0],
    ...
    for _ in range(1):
        for i in range(0, 5):
            ...
            if 0 == ret[0]:
                print('move_lineb, ret = %d' % (ret[0],), time.time() - start)
